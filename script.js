...
var dayWeek = moment().format('dddd, MM/Do/YYYY hh:mm:ss a');
$('#currentDay').text(dayWeek);
console.log(dayWeek);

$('#container').append(dayWeek);
...
//   saving input to local storage after the save butt
  ...
  $(".saveBtn").on("click", function (event) {
    ...
    localStorage.setItem(event.target.attributes[0].value, dayWeek);
  });
  
  $(document).ready(function () {
    if (localStorage["9am"] !== null && localStorage["9am"] !== undefined) {
      var nineAm = $("<p>" + localStorage["9am"] + "</p>");
      $(".form-control").append(nineAm[0].innerText);
    } else {
      ("");
    }
